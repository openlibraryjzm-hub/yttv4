...
const RadialMenuStandalone = ({ 
  ...
  useEffect(() => {
    ...
    // Reset startIndexRef from visibleStartIndex config
    ...
    bundles.forEach(bundle => {
      ...
      if (useScrollingList) {
        ...
        // Top teleport: last item (highest index) - will come in when scrolling down
        if (topTeleportContainer && bundleItemsList.length > 0) {
          const lastItem = bundleItemsList[bundleItemsList.length - 1]
          allNewContentElements.push({
            id: `teleport-top-${topTeleportContainer.id}-${lastItem.id || bundleItemsList.length - 1}`,
            bundleId: bundle.id,
            itemIndex: bundleItemsList.length - 1,
            containerId: topTeleportContainer.id,
            text: lastItem.text || `item${bundleItemsList.length}`,
            color: lastItem.color || topTeleportContainer.contentColor || topTeleportContainer.color || '#ffffff',
            fontColor: lastItem.fontColor || topTeleportContainer.fontColor || '#000000',
            imageUrl: lastItem.imageUrl || topTeleportContainer.imageUrl || null,
                    ...
                    Promise.all([fadeOutAndMorph, ...shiftAnimations, fadeInAndMorph]).then(() => {
                      ...
                      setContentElements(prev => {
                        ...
                        if (topTeleportElement.itemIndex !== undefined) {
                          nextTopItemIndex = (topTeleportElement.itemIndex - 1 + bundleItemsList.length) % bundleItemsList.length
                        }
