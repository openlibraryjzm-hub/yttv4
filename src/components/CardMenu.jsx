    ...
    <div ref={containerRef} className="relative" data-card-menu="true">
      ...
      {isOpen && (
        <div className="relative" style={{ zIndex: 9999 }}>
          <div
            ref={menuRef}
            className="absolute bottom-full right-0 mb-2 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-[9999] overflow-visible"
          ...
          {activeSubmenu && submenuOptions[activeSubmenu] && (
            <div
              className="absolute top-0 right-full mr-2 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-[10000] overflow-visible"
              onClick={(e) => e.stopPropagation()}
              style={{ zIndex: 10000 }}
            >
              {/* Back button */}
              <button
                ...
                className="w-full text-left px-4 py-2.5 text-sm text-slate-400 hover:bg-slate-700 border-b border-slate-700"
              >
                <div className="flex items-center gap-3">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {submenuOptions[activeSubmenu].map((submenuOption, index) => (
                <button
                  key={index}
                  onClick={() => !submenuOption.disabled && handleSubmenuOptionClick(activeSubmenu, submenuOption)}
